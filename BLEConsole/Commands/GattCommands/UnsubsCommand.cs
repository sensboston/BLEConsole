        ...
        private async Task UnsubscribeAll(BleContext context)
        {
            foreach (var subscriber in context.Subscribers)
            {
                try
                {
                    ...
                    // TODO: Need to properly remove event handler
                    // subscriber.ValueChanged -= Characteristic_ValueChanged;
                    // This requires event handler to be stored in context or passed as parameter
                }
                catch (Exception ex)
                {
